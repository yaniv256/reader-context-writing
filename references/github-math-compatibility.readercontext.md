...
Reader model: A writing agent knows GitHub advertises LaTeX-style mathematics through MathJax but needs a deployment-safe subset rather than a general promise.
Does not know: Which commands this workflow has actually rendered, which standard-looking commands fail, or how a new failure changes the contract.
Cares about: Preventing a technically valid source document from publishing broken or misleading mathematics on GitHub.
Does not care about: Untested claims that a macro should work because upstream MathJax supports it.
Reader voice: "What can I safely use today, and what proof is required before I add another command?"
Unresolved: Official scope, empirical subset, recurring hazards, and release gate.
Next passage: Separate GitHub's public promise from this workflow's verified compatibility registry.
Do not assume: That GitHub's Markdown preprocessing passes TeX input unchanged to MathJax.
-->
