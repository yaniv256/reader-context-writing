...
<!--
READER CONTEXT SIDECAR
Survival: 91% — the loop section gave the agent a drafting routine and a template with a Survival field it has not yet been told how to fill. That is an open question the agent is holding, so this section is wanted rather than tolerated.
Reader model: The agent can draft passage by passage and records reader state, including a Survival figure whose scale and purpose it has not been given.
Does not know: What the figures mean, how to calibrate them honestly, or that they compose into a hard gate on the document.
Cares about: A rule that fails loudly, so that "this section is a bit long" becomes a number it can be held to.
Does not care about: The history of how the budget was arrived at.
Reader voice: "A percentage per paragraph sounds arbitrary. What stops me from writing 95% everywhere and declaring victory?"
Unresolved: The calibration, the anti-gaming rule, and what to do when a section fails.
Next passage: State the invariant first as a budget, because the number per passage only matters through what it compounds to.
Do not assume: That the agent will infer that conditional figures multiply, or that it will resist scoring its own prose generously.
-->
